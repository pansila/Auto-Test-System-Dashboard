    ...
    <el-form ref="form" v-loading="listLoading" :model="form" label-width="120px">
      ...
      <el-form-item label="Parallization">
        <el-radio-group v-model="form.parallelization">
          <el-radio label="0">Run on any of endpoints</el-radio>
          <el-radio label="1">Run on all endpoints</el-radio>
        </el-radio-group>
      </el-form-item>
      ...
      for (const idx in this.fileList) {
        ...
        try {
          ...
          if (response.data.status === 0) {
            this.resource_id = response.data.data
          } else {
            this.$message({
              message: `Failed to upload files, status: ${response.data.status}`,
              type: 'error'
            })
