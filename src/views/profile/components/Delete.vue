<template>
  <div>
    <el-table
      ...
      stripe
    >
      <el-table-column :label="$t('profile.organizationTeam')">
        <template slot-scope="scope">
          {{ scope.row.label }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('permission.transfer')">
        <template slot-scope="scope">
          <el-select v-model="new_owners[scope.row.label]" :placeholder="$t('profile.placeholderTransfer')">
            <el-option
      ...
      <el-table-column :label="$t('profile.action')">
        <template slot-scope="scope">
          <el-button size="small" @click="onTransfer(scope.row)">{{ $t('profile.transferOwnership') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-form :model="form" :rules="passwordRules">
      ...
      <el-form-item>
        <el-button type="danger" @click="dialogVisible = true">{{ $t('permission.delete') }}</el-button>
      </el-form-item>
    </el-form>
    <el-dialog
      title="Warning"
